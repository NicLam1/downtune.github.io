<template>
  <div class="background-wrapper" :style="{ background: backgroundGradient }">
    <div class="container-sm py-4 position-relative quiz-container">
        ...
        <div :key="currentQuestion.id" class="mb-4">
          ...
          <div class="row">
            <div v-for="(option, index) in currentQuestion.options" :key="index" class="col-6">
              <button class="w-100 p-3 my-1" :class="isSelected(option) ? 'btn btn-secondary' : 'btn btn-primary'"
      ...
      <!-- Continue button -->
      <button v-if="currentQuestionIndex < questions.length" class="btn btn-success w-100" @click="handleContinue"
...
.background-wrapper {
  min-height: 100vh;
  /* Full screen height for the wrapper */
  display: flex;
...
.quiz-container {
  ...
  width: 50%;
  /* Container width is 50% of the page */
  min-height: 50vh;
  /* Minimum height is 50% of the viewport */
...
.col-6 {
  width: 48%;
  /* Ensure two options fit in each row */
}
...
@media (max-width: 768px) {
  .quiz-container {
    width: 90%;
    /* On smaller screens, make the container wider */
  }

  .col-6 {
    width: 100%;
    /* On smaller screens, make each option span the full width */
  }
}
</style>